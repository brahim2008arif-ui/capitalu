...
const STORAGE_KEY = 'clc_app_user_state_v3';
...
export default function App() {
  ...
  const handleOpenSupport = () => {
    showToast(
      language === 'ar'
        ? 'خدمة العملاء متصلة 24/7. جارٍ تحويلك...'
  ...
  const handleShowCompanyInfo = () => {
    showToast(
      language === 'ar'
        ? 'ترخيص شركة Capital Luxury Cars لندن برقم #CL982341.'
  ...
  const handleCompleteTask = () => {
    if (!isTaskAvailable) {
      showToast(
        language === 'ar' ? 'المهمة قيد الانتظار لليوم!' : 'Task is in cooldown for today!'
      );
      return;
    }
    ...
    showToast(
      language === 'ar'
        ? 'تم استلام أرباح المهمة +1.50 USDT!'
  ...
  const handleConfirmPayment = (payment: PaymentRequest, txId: string) => {
    const newRecord: TransactionRecord = {
      id: String(Date.now()),
      type: payment.type === 'upgrade' ? `ترقية (${payment.level})` : 'إيداع رصيد',
      amount: payment.cost,
      date: new Date().toISOString().replace('T', ' ').substring(0, 16),
      status: 'Pending (24h)',
      note: `TxID: ${txId.slice(0, 10)}... (مدة التأكيد 24 ساعة)`,
    };
    ...
    showToast(
      language === 'ar'
        ? `تم تقديم الدفعة! فترة المراجعة 24 ساعة لتأكيد ${payment.level}.`
  ...
  const handleSubmitWithdrawal = (amount: number, address: string, method: string) => {
    ...
    const newRecord: TransactionRecord = {
      ...
      date: new Date().toISOString().replace('T', ' ').substring(0, 16),
      status: 'Pending (24h)',
      note: `Address: ${address.slice(0, 8)}... (فترة المعالجة 24 ساعة)`,
    };
    ...
    showToast(
      language === 'ar'
        ? 'تم تقديم طلب السحب! مدة التنفيذ والمعالجة 24 ساعة.'
  ...
  const handleSpinLuckyWheel = (wonAmount: number) => {
    ...
    showToast(
      language === 'ar'
        ? `مبروك! ربحت +${wonAmount.toFixed(2)} USDT من عجلة الحظ!`
      ...
      <div className="w-full max-w-md bg-[#fcfbfb] min-h-screen flex flex-col relative shadow-2xl pb-24">
        ...
        <div className="bg-gray-900 text-amber-300 px-4 py-2 text-[11px] font-semibold flex items-center justify-between border-b border-gray-800">
          <span className="flex items-center gap-1.5">
            ...
            <span>{language === 'ar' ? 'مدة السحب وتأكيد الإيداع: 24 ساعة عمل' : 'Withdrawal & Payment Period: 24 Hours'}</span>
          </span>
