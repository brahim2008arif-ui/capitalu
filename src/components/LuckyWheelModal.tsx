...
export const LuckyWheelModal: React.FC<LuckyWheelModalProps> = ({
  ...
  const handleSpinClick = () => {
    ...
    // Sector center in wheel space is (targetSectorIndex * 45 + 22.5) deg from 12 o'clock clockwise.
    // To position this sector at 12 o'clock (under the top needle), the wheel must end at:
    // (360 - (targetSectorIndex * 45 + 22.5)) mod 360
      ...
      <div className="w-full max-w-sm text-center text-white my-auto">
          ...
          >
            {spinning ? (
              ...
              <>
                ...
                <span className="text-[9px] leading-tight font-extrabold">{language === 'ar' ? '24 س' : '24h'}</span>
              </>
            )}
          </button>
        </div>
        ...
        {lastWon !== null && !spinning && (
          <div className="mt-1 py-2 px-4 bg-amber-400 text-gray-950 rounded-xl font-black text-xs inline-flex items-center gap-1.5 shadow-lg border border-amber-300 animate-pulse">
            ...
            <span>
              {language === 'ar'
                ? `مبروك! ربحت +${lastWon.toFixed(2)} USDT`
        ...
        <div className="bg-white text-gray-800 rounded-3xl p-4 mt-3 text-xs shadow-xl border border-gray-100 text-start">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-[10px] text-gray-400 font-semibold flex items-center gap-1">
              ...
              <span>{language === 'ar' ? 'تتجدد كل 24 ساعة' : 'Refills every 24h'}</span>
            </span>
          </div>
          <p className="text-[11px] text-gray-500 leading-relaxed">
            {language === 'ar'
              ? 'تتجدد فرصتك المجانية تلقائياً كل 24 ساعة بعد التدوير! كما يمكنك دعوة الأصدقاء أو ترقية مستوى باقتك VIP لربح مكافآت مستمرة إضافية.'
          ...
          <button
            type="button"
            onClick={onClose}
            className="mt-2.5 text-red-600 font-bold text-xs hover:underline block"
          >
            {t.continueToDash} →
          </button>
        </div>
