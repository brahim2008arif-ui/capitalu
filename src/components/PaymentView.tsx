...
export const PaymentView: React.FC<PaymentViewProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!txId.trim()) {
      onShowToast(
        language === 'ar'
          ? 'يرجى إدخال رقم المعاملة (TxID) أو الهاش'
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="font-bold text-gray-900 text-base">
            {language === 'ar' ? 'مركز الدفع وتأكيد الحوالة' : 'Payment & Upgrade Center'}
          </h3>
          <p className="text-[11px] text-gray-500">
            {language === 'ar' ? 'مدة معالجة الدفع: 24 ساعة كحد أقصى' : 'Payment Confirmation Period: 24h'}
          </p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-gray-900 via-gray-850 to-red-950 text-white rounded-3xl p-5 shadow-lg border border-gray-800">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-[10px] text-red-300 font-bold uppercase tracking-wider bg-white/10 px-2.5 py-0.5 rounded-full">
              {payment.type === 'upgrade'
                ? language === 'ar' ? 'طلب ترقية الباقة' : 'Account Upgrade Order'
                : language === 'ar' ? 'طلب إيداع رصيد' : 'Recharge Order'}
            </span>
          ...
          <div className="text-end">
            <span className="text-xs text-gray-300 block">{language === 'ar' ? 'المبلغ المطلوب' : 'Amount Due'}</span>
        ...
        <div className="mt-4 pt-3 border-t border-white/10 flex items-center gap-2 text-xs text-amber-200 bg-white/5 p-2.5 rounded-2xl border border-white/5">
          ...
          <span className="leading-tight">
            {language === 'ar'
              ? 'مدة تأكيد الدفع ومراجعة المعاملة تتم خلال 24 ساعة من تاريخ الإرسال.'
      ...
      <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 space-y-4">
        <div>
          <label className="block text-xs font-bold text-gray-800 mb-2">
            {language === 'ar' ? 'اختر شبكة الدفع (USDT):' : 'Select Payment Network (USDT):'}
          </label>
        ...
        <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs font-bold text-gray-700">
              {language === 'ar' ? 'عنوان الإيداع المخصص:' : 'Deposit Address:'}
            </span>
          ...
          <p className="text-[11px] text-amber-700 mt-2 flex items-center gap-1">
            ...
            <span>
              {language === 'ar'
                ? 'قم بإرسال عملة USDT فقط عبر هذه الشبكة المحددة.'
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label htmlFor="payment-txid-input" className="block text-xs font-bold text-gray-700 mb-1.5">
              {language === 'ar' ? 'رقم المعاملة (TxID / Hash):' : 'Transaction Hash (TxID):'}
            </label>
            <input
          ...
          >
            ...
            <span>
              {language === 'ar'
                ? `تأكيد الدفع (فترة المراجعة 24 ساعة)`
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-5 text-center shadow-2xl border border-emerald-100">
            ...
            <h3 className="font-black text-gray-900 text-base">
              {language === 'ar' ? 'تم تقديم الدفعة بنجاح' : 'Payment Submitted Successfully'}
            </h3>
            <p className="text-xs text-gray-600 mt-2 leading-relaxed">
              {language === 'ar'
                ? 'فترة تأكيد الدفع هي 24 ساعة. سيتم التحقق من المعاملة عبر البلوكشين وتفعيل ' + payment.level + ' تلقائياً.'
            ...
            <div className="bg-gray-50 rounded-2xl p-3 my-3 text-xs border border-gray-100 text-start space-y-1">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">{language === 'ar' ? 'فترة المعالجة:' : 'Period:'}</span>
            ...
            >
              {language === 'ar' ? 'حسناً، تم الفهم' : 'OK, Got It'}
            </button>
          </div>
        </div>
      )}
