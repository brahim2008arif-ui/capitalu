...
export const WithdrawalView: React.FC<WithdrawalViewProps> = ({
  ...
  const handleConfirm = (e: React.FormEvent) => {
    ...
    if (numAmount > userState.balance) {
      onShowToast(
        language === 'ar'
          ? 'المبلغ المطلوب يتجاوز رصيدك المتاح!'
    ...
    if (!address.trim()) {
      onShowToast(
        language === 'ar'
          ? 'يرجى إدخال عنوان محفظة السحب الخاصة بك!'
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-red-600 font-semibold flex items-center gap-1">
            <span>فترة معالجة السحب: 24 ساعة عمل</span>
          </p>
        </div>
      </div>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <h4 className="font-extrabold text-amber-900 text-sm">
                  {language === 'ar' ? 'تنبيه: مطلوب الترقية إلى باقة VIP1' : 'Notice: Upgrade to VIP1 Required'}
                </h4>
              </div>
              <p className="text-xs text-amber-800 mt-1 leading-relaxed">
                {language === 'ar'
                  ? 'السحب مقفل لحسابات باقة VIP0. يجب ترقية حسابك إلى VIP1 لفتح إمكانية سحب الأرباح.'
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-emerald-900">
              {language === 'ar' ? `حساب مؤهل (${userState.vipLevel}) - السحب متاح` : `Active Account (${userState.vipLevel}) - Withdrawals Enabled`}
            </p>
            <p className="text-[10px] text-emerald-700">
              {language === 'ar' ? 'يمكنك سحب أرباحك مباشرة إلى محفظتك' : 'You can withdraw funds directly to your wallet'}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-2 p-2.5 bg-red-50/80 border border-red-100 rounded-2xl text-xs text-red-800 mb-4">
          ...
          <span className="font-medium">
            {language === 'ar'
              ? 'مدة معالجة وتنفيذ الحوالة خلال 24 ساعة من تاريخ الطلب.'
        ...
        <form onSubmit={handleConfirm} className="space-y-4">
          ...
          <div>
            <label htmlFor="withdrawal-address-input" className="block text-xs font-bold text-gray-700 mb-1.5">
              {language === 'ar' ? 'عنوان محفظة السحب' : 'Withdrawal Wallet Address'}
            </label>
            <input
              ...
              placeholder={language === 'ar' ? 'عنوان المحفظة (USDT)' : 'Enter wallet address'}
          ...
          <div>
            <label htmlFor="withdrawal-password-input" className="block text-xs font-bold text-gray-700 mb-1.5">
              {language === 'ar' ? 'كلمة مرور المعاملة' : 'Security / Login Password'}
            </label>
            <input
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-5 text-center shadow-2xl animate-scale-up border border-red-100">
            ...
            <h3 className="font-black text-gray-900 text-lg">
              {language === 'ar' ? 'مطلوب ترقية الحساب إلى VIP1' : 'VIP1 Upgrade Required'}
            </h3>
            <p className="text-xs text-gray-600 mt-2 leading-relaxed">
              {language === 'ar'
                ? 'السحب غير مسموح به في المستوى التجريبي VIP0. يرجى الترقية إلى VIP1 على الأقل لسحب أرباحك إلى محفظتك!'
            ...
            <div className="bg-gray-50 rounded-2xl p-3 my-4 border border-gray-100 text-start text-xs space-y-1">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">{language === 'ar' ? 'تكلفة الترقية:' : 'Upgrade Price:'}</span>
