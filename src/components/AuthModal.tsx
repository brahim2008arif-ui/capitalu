...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSignIn = () => {
    ...
    if (!trimmedId) {
      onShowToast(
        language === 'ar'
          ? `يرجى إدخال ${authMethod === 'phone' ? 'رقم الهاتف' : 'البريد الإلكتروني (Gmail)'}`
          : `Please enter your ${authMethod === 'phone' ? 'phone number' : 'Gmail / email'}`
      );
      return;
    }
    if (!trimmedPass) {
  ...
  const handleSignUp = () => {
    ...
    if (authMethod === 'email' && !trimmedId.includes('@')) {
      onShowToast(
        language === 'ar'
          ? 'يرجى إدخال بريد إلكتروني صالح (Gmail أو غيره)'
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-md border border-gray-100">
          ...
          <div className="flex border-b border-gray-100 mb-4">
            ...
            >
              {language === 'ar' ? 'البريد الإلكتروني (Gmail)' : 'Gmail / Email'}
            </button>
            <button
              type="button"
            ...
            <div className="mb-4 bg-gray-50 rounded-2xl p-3 border border-gray-100">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-bold text-gray-700 flex items-center gap-1">
                  ...
                  <span>
                    {language === 'ar'
                      ? 'الحسابات المسجلة مسبقاً في النظام:'
              ...
              <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
                {registeredUsersList.map((user) => (
                  <button
                    ...
                    title={language === 'ar' ? 'اضغط لتسجيل الدخول السريع بهذا الحساب' : 'Click to sign in with this account'}
                  >
          ...
          <div className="mt-5 space-y-2.5">
            ...
            <div className="pt-2 text-center">
              {authMode === 'signin' ? (
                ...
                >
                  {language === 'ar'
                    ? 'ليس لديك حساب؟ سجّل حساباً جديداً الآن'
                ...
                >
                  {language === 'ar'
                    ? 'لديك حساب بالفعل؟ سجّل دخولك من هنا'
